...
// Nuevo Isotipo: Rayo / Energía / Innovación (Cyber Green)
...
export function SoftnetMark({
  ...
  return (
    <span
      className={cn(
        "relative isolate inline-flex items-center justify-center overflow-hidden rounded-none border border-primary/30",
        variant === "solid"
          ? "bg-black shadow-[4px_4px_0_0_rgba(0,255,65,0.1)]"
          : "bg-black/80 backdrop-blur-md shadow-sm",
        className
      )}
    >
      <span className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(0,255,65,0.05),transparent_40%),radial-gradient(circle_at_80%_80%,rgba(0,255,65,0.05),transparent_40%)]" />
      <span className="absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,transparent,rgba(0,255,65,0.1),transparent)]" />
  ...
  return (
    <div className={cn("inline-flex items-center gap-3.5", className)}>
      ...
      <div>
        ...
        >
          Innova <span className="text-primary">Systems</span>
        </span>
...
export function SoftnetMotif({ className, tone = "dark" }: SoftnetMotifProps) {
  const stroke = tone === "dark" ? "rgba(0, 255, 65, 0.4)" : "rgba(0, 255, 65, 0.2)"
  const strokeStrong = tone === "dark" ? "rgba(0, 255, 65, 0.8)" : "rgba(0, 255, 65, 0.5)"
  const dot1 = "#00FF41"
  const dot2 = "#10B981"
  const dot3 = "#059669"
...
export function SoftnetBrandNote({
  ...
  return (
    <div className={cn("code-caption font-mono text-[0.66rem] font-semibold text-primary/80 uppercase tracking-widest", className)}>
