...
const ProceduralGroundBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Color Palette
        vec3 baseColor = vec3(0.02, 0.01, 0.05); // Deep Dark Space
        vec3 accentColor = vec3(0.0, 0.8, 0.2);   // Electric Green (Innova Primary)
        vec3 neonColor = vec3(0.0, 1.0, 0.25);    // Neon Green
        ...
        // Horizon Fog / Fade
        float fade = smoothstep(0.1, -0.8, uv.y);
  ...
  return (
    <div className="absolute inset-0 w-full h-full bg-[#020105] -z-10">
      <canvas
        ref={canvasRef}
